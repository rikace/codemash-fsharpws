...
module AgentWebCrawler =
    ...
    (*
    MODULE 1    Web-Crawler        (FSharpWebCrawler.AgentWebCrawler)
        BONUS        Error handling + supervision
    MODULE 2    Web-Crawler Parallel
    MODULE 3    Agent compoistion (FSharpWebCrawler.MessagePassing.AgentPipeline) >=> (+ parallelism)
        ...
        let broadcastAgent () =
            parallelAgent parallelism (fun inbox ->
                let rec loop (agents : Agent<_> list) = async {
                    let! msg = inbox.Receive()
                    match msg with
                    // the message is sent (broadcast) to all the agents subscribed to this agent.
                    // The agent registration is done using the "Mailbox(agent)" message/case.
                    // The list of agent subscribed is kept as state of the agent loop (agents : Agent<_> list)
                    | Item(item) ->
                        for agent in agents do
                            agent.Post(Item(item))
